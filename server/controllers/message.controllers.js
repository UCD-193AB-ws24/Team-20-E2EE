...
export const getChatArchive = async (req, res) => {
  try {
    const { senderUid, recipientUid } = req.query;

    if (!senderUid || !recipientUid) {
      return res.status(400).json({ error: "Both senderUid and recipientUid are required" });
    }
    ...
    const messages = await messagesCollection.find({
      $or: [
        { sender: senderUid, recipient: recipientUid },
        { sender: recipientUid, recipient: senderUid }
      ]
    }).sort({ timestamp: 1 }).toArray();
